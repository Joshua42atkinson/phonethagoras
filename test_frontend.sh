...
if ! grep -q '<script src="js/ai.js"></script>' "$FRONTEND_DIR/src/index.html"; then
  echo "❌ FAIL: ai.js script tag missing in index.html"
  exit 1
fi

if ! grep -q '<script src="js/chat.js"></script>' "$FRONTEND_DIR/src/index.html"; then
  echo "❌ FAIL: chat.js script tag missing in index.html"
  exit 1
fi

if ! grep -q 'id="chat-document-viewer"' "$FRONTEND_DIR/src/index.html"; then
